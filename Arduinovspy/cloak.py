...
def create_mask(frame, lower_color, upper_color):
    ...
    mask = cv2.morphologyEx(mask, cv2.MORPH_OPEN, np.ones((6, 6), np.uint8), iterations=2)
    mask = cv2.morphologyEx(mask, cv2.MORPH_DILATE, np.ones((2, 2), np.uint8), iterations=1)
    return mask
